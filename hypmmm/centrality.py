...
# from Mortality import centrality_utils
from hypmmm import centrality_utils
...
def comp_adjacency(
    incidence_matrix, edge_weight, node_weight, rep="standard", weight_resultant=True
):
    """
    This function computes the adjacency matrix for an undirected hypergraph for both the standard
    and dual representations, allowing the resultant adjacency matrix to be weighted by both
    edges and nodes if user chooses.
    ...
    Args:
        ...
        which must have the same number of elements as the second axis of matrix.

        node_weight (np.array, dtype=np.float64) : A vector of node weights,
        which must have the same number of elements as the first axis of matrix.

        weight_resultant (bool) : This flag tells the function whether to include
        both edge and node weights in the adjacency matrix calculation. If this is
        set to True then the matrix is computed as of sqrt(W_a) M^T W_b M sqrt(W_a),
        where a and b represent either edge/node weight or node/edge weight matrices.
        If set to False, adjacency matrix calcultes as M^T W_b M

        rep (string) : Type of adjacency matrix to compute, if rep="standard", then
        adjacency matrix is size of first axis of matrix, i.e. of the nodes of the
        hypergraph. If rep="dual", then adjacency matix is size of second axis, i.e.
        of the number of edges in the hypergraph.
    """

    # 1) Set up hypergraph type
    if rep == "standard":
    ...
    # 1) M W
    weighted_incidence = np.zeros((nrows, ncols), dtype=np.float64)
    for i in range(nrows):
        for j in range(ncols):
            weighted_incidence[i, j] += inc_mat[i, j] * weight[j]

    # 2) M W M^T
    intermediate = np.zeros((nrows, nrows), dtype=np.float64)
    for i in range(nrows):
    ...
    subt = np.zeros(nrows, dtype=np.float64)
    for i in range(nrows):
        for k in range(ncols):
...
def eigenvector_centrality(
    ...
    the dual hypergraph of the undirected representation of the directed hypergraph.
    ...
    Args:
        ...
        rep (string, optional) : The representation of the hypergraph for which to
        calculate the eigenvector centrality. Options are "standard" or "dual".
        (default: "standard").

        tolerance (float, optional) :The difference between iterations in the
        eigenvalue at which to assume the algorithm has converged (default: 1e-6).

        max_iterations (int, optional) : The maximum number of iterations (of the
        power method) to perform before terminating the algorithm and assuming it
        has not converged (default: 100).

        weight_resultant (bool) : Flag to tell function to include the node weights
        during computation of the transition matrix for the dual directed hypergraph.

        random_seed (int, optional) : The random seed to use in generating the
        initial vector (default: 12345).

        return_error (bool) : Flag to return eigenvector and eigenvalue, its error
        and the iteration of convergence.
    """
    # 1) setup up hyergraph type and initialise eigenvector
    ...
    for iteration in range(max_iterations):
        ...
        # To estimate eigenvalue and error, take ratio of new to old eigenvector
        # ignoring zeros, whose mean and standard deviation represent new estimate and error
        ...
        if eigenvalue_error_estimate / eigenvalue_estimate < tolerance:
            ...
            print(
                f"""\nConverged at largest eigenvalue {eig_est:g} ± {eig_err:f} after {iteration} iterations"""
            )
    ...
    # Break out of algorithm if maximum iterations were reached
        ...
        print(
            f"Last estimate was {float(f'{eig_est:.3g}'):g} ± {float(f'{eig_err:.3f}'):f}"
        )
    ...
    if return_error:
        output = eigenvector, (eigenvalue, eigenvalue_error_estimate), iteration
    else:
        output = eigenvector
...
def comp_ptm_std(
    inc_mat_tail, inc_mat_head, weights, n_deg, e_deg=None, rep="successor", eps=0.00001
):
    """
    ...
    If representation is for successor detection:
    ...
    A random walker may transition from node u to v if they share a hyperarc e
    where u is the head set and v is in its tail set, i.e. we move in the opposite
    direction of the hyperarc.

    In matrix notation, this function calculates P = D+v^{-1} M+ We D-e^{-1} M-^T
    ...
    Args:
        ...
        rep (str) : Either "sucessor" or "predecessor" to define which transition
        rule is used to compute transition probability matrix

        eps (float) : Small value to prevent zero division error. Only applied
        to node degrees as it is assumed edge degrees are non-zero due to non-zero
        edge weights and edges which always have nonzero tail and head nodes.
    """
    ...
    # 2) Compute [node_degree * inc_mat(+/-) * edge_weights] * inc_mat(-/+) (* edge degree)
    ...
    for i in range(n_diseases):
        ...
        # For any nodes where they were never a successor/predecessor, i.e. their
        # row is entirely 0, set the self-returning probability to 1 - causing a
        # reducible PTM
...
def pagerank_centrality(
    ...
    Args:
        ...
        arrays for computing PageRank vector.
        ...
        typ="successor" then node transition is in direction of observed disease
        progression. If typ="predecessor", then node transition is in inverse
        direction of observed disease progression.
        ...
        return_error (bool) : Flag to return eigenvector, but also eigenvalue, its error
        and the iteration of convergence.
    """
    ...
    if rep == "standard":
    ...
    elif rep == "dual-simple":
        ptm = centrality_utils.comp_ptm_dual_simp(
            inc_mat_head,
            inc_mat_tail,
            edge_weights,
            node_degree_tail,
        ...
        old_eigenvector_estimate = rng.random(n_edges, dtype="float64")

    elif rep == "dual-prog":
        ptm = centrality_utils.comp_ptm_dual_prog(
            inc_mat_head,
            inc_mat_tail,
            edge_weights,
            node_degree_tail,
        ...
        ptm = centrality_utils.comp_ptm_dual_reg(
            inc_mat_head,
            inc_mat_tail,
            edge_weights,
            node_degree_head,
            ...
            if verbose:
                print(
                    "\nConverged at largest eigenvalue {:.2f} ± {:.4f} after {} iterations".format(
                        eigenvalue_estimate, eigenvalue_error_estimate, iteration
                    )
                )
            break
